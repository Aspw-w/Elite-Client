...
class NoSlow : Module("No Slow", ModuleCategory.Movement, GLFW.GLFW_KEY_UNKNOWN, false, true) {
    ...
    override fun onUpdate(event: UpdateEvent) {
        ...
        if (mode.get().equals("hypixel", true) && waitingPacket && MovementUtil.fallTicks >= 3 && (IMinecraft.mc.player!!.mainHandStack.item.components.contains(DataComponentTypes.FOOD) || IMinecraft.mc.player!!.mainHandStack.item == Items.POTION || IMinecraft.mc.player!!.mainHandStack.item is BowItem || IMinecraft.mc.player!!.mainHandStack.item == Items.MILK_BUCKET)) {
